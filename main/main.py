...
def create_absa_input_from_community(community_csv_path, absa_input_path):
    ...
    with open(community_csv_path, newline="", encoding="utf-8") as infile, \
        ...
        for row in reader:
            # Make sure 'id' and 'text' columns exist in community_input.csv
...
def main():
    ...
    # NEW: model architecture and specific model name
    parser.add_argument("--model", choices=["gemini", "pyabsa"], default="gemini",
                        help="Main model/algorithm for ABSA (gemini or pyabsa)")
    parser.add_argument("--model_name",
                        help="Specific model checkpoint (e.g., gemini-2.5-pro, multilingual)")
    ...
    parser.add_argument("--conf_threshold", type=float, default=0.6,
                        help="Confidence threshold for sentiment classification for Gemini, ignored for PyABSA")
    ...
    # Auto-create absa_input.csv from community_input.csv
    ...
    elif args.model == "pyabsa":
        model_value = args.model_name if args.model_name else "multilingual"
        run_cmd([
            "pyabsa",
            "--input", absa_input_csv,
            "--aspects", args.absa_aspect,
    ...
    if args.model == "gemini":
        ...
        # PyABSA output file
        matching_files = glob.glob(os.path.join(aspect_output_folder, "*.csv"))
        absa_output_csv = matching_files[0] if matching_files else None
