...
def calculate_consensus(user_pairs, aspects_data, p=2):
    """Calculate AC values and CC from given user pairs, skipping zero–zero pairs,
       and normalizing CC over only active aspects in this community."""
    if not user_pairs:
        ...
        if u in aspects_data and v in aspects_data:
            ...
            for k in range(m):
                ...
                if np.allclose(v1, 0) and np.allclose(v2, 0):
                    sims.append(None)  # skip both zero
                else:
    ...
    for z in range(m):
        ...
        if sims_for_aspect:
            ...
            ac_values.append(0.0)  # keep placeholder for reporting

    if active_aspects == 0:
        cc = 0.0
    else:
        # Normalize only over active aspects
